...
inputfolder = r'C:\Users\thyan\Documents\Wind Transmission Project - Local\5th Sep'

for folders in os.listdir(inputfolder):
    print(folders)
    state_Name = abbrev_us_state[folders]
    state_abbr = folders
    outputFolder = inputfolder + '\\' + folders + '\\' + state_Name
    workingfolder = inputfolder + '\\' + folders
    os.makedirs(outputFolder)
    for file in os.listdir(workingfolder):
        if file.endswith('.tif'):
            file_path = workingfolder + '/' + file
            print(file_path)
